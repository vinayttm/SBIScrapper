package com.app.sbiscrapper.Utils
   ...
   public fun findNodesByContentDescription(rootNode: AccessibilityNodeInfo?, targetContentDescription: String): List<AccessibilityNodeInfo> {
    ...
    fun findAndScrollListView(
        ...
        if (node != null) {
            ...
            for (i in 0 until childCount) {
                val childNode = node.getChild(i)
                if (childNode != null && className == childNode.className) {
                    return childNode
                }
                val resultNode = findAndScrollListView(childNode, className)
                if (resultNode != null) {
                    return resultNode
                }
            }
